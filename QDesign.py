...
class AI_Application_Design(QMainWindow):
    ...
    def insert_supervised_model(self, model):
        ...
        def reveal_model(model_data):
            ...
            path = fr"Database/archive/static/img/models/img1/{name}1.png".replace(' ', '')
            print(path)
            pixmap = QPixmap(path)
            ...
            label_bin = QLabel()
            label_bin.setStyleSheet("text-align:center;")
            path = fr"Database/archive/static/img/models/img2/{name}2.png".replace(' ', '')
            print(path)
            pixmap = QPixmap(path)
            pixmap = pixmap.scaled(300, 300, Qt.KeepAspectRatio)
            label_bin.setFixedHeight(pixmap.height())
    ...
    def insert_dir_in_project_files(self, dir_, file_full_path=None):
        ...
        def start_file(e):
            os.startfile(fr"Database\\Archive\\projects\\{self.project_id}\\{self.focusWidget().path_}")
        ...
        cur.close()
        List = ['None'] + list(QTools.read_file(f"Database\\archive\\projects\\{self.project_id}\\{dir_}").columns)
        label_combo.setItems(List)
        label_combo.setCurrentIndex(List.index(label.__str__()))
        label_combo.path_ = f"Database\\archive\\projects\\{self.project_id}\\{dir_}"
    ...
    def insert_file_in_project(self, file_full_path, project_id, url=False):
        try:
            ...
            self.db.commit()
            cur.close()
            file = open(f"Database\\Archive\\projects\\{project_id}\\{file_id}.{file_full_path.split('.')[-1]}", 'wb')
            file.write(data)
            file.close()
    ...
    def seek_files_in_project(self, id_):
        dirs = os.listdir(fr"Database\\Archive\\projects\\{id_}")
        if dirs:
            for dir_ in dirs:
            ...
            pixmap = QPixmap(r"C:\Users\lenovo\Desktop\AIProject\Database\archive\static\img\nothing_here_to_show_2.png")
    ...
    def workbench_page(self, creation_date, project_description, project_name, id_):
        ...
        self.sheet_place.setStyleSheet("border-radius:10px; background-color:#000; background-image:url(Database/archive/static/img/sheet.png); background-repeat:no-repeat; background-position: center;")
    ...
    def connect_database(self):
        self.db = sql.connect('Database\\database.sqlite3')
    ...
    def seek_projects_in_database(self):
        ...
        if data:
            ...
            pixmap = QPixmap(r'Database\archive\static\img\nothing_here_to_show.png')
    ...
    def main_page(self):
        ...
        def create_project_button_f(e):
            ...
            if project_name:
                ...
                os.mkdir(fr"Database\\Archive\\projects\\{id_}")
            else:
                msg_box = QMessageBox()
                msg_box.setStyleSheet('background-color:#333; color:#fff;')
                msg_box.setIcon(QMessageBox.Warning)
