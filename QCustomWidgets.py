...
class CustomContextMenuButton(QPushButton):
    ...
    def showContextMenu(self, pos):
        ...
        if selected_action == action1:
            ...
            self.sheet.setWindowTitle(fr"AI Algorithms Sheets - {self.mainclass.sheet.file_path}")
            self.sheet.show()
...
class QSheet(QScrollArea):
    ...
    def load_pandas_dataframe(self, df: pd.DataFrame, label, file_path, file_id):
        ...
        if len(headers) > len(set(headers)):
            ...
            for header_index in range(len(headers)):
                ...
                header_input = CustomLineEdit(header.__str__())
                self.HEADERS.append(header_input)
                header_input.mainclass = self
                header_input.setAlignment(Qt.AlignCenter)
